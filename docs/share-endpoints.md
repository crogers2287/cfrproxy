...
- **Thinking level** — `reasoning_effort` (`off|low|medium|high|xhigh`) sets the reasoning effort sent upstream for this share, overriding the provider's own default. It fills in only when the caller sent no level of its own; `reasoning_force` replaces the caller's level too. The share is the place to do this: the level lands in the rendered prompt head, so it must be stable per client for the prefix cache to stay warm. Qwen3.8 chat templates default to **xhigh** when a request carries nothing, which is why every harness turn on a local model looked like xhigh in llama-swap until this existed. `off` is expressed as `chat_template_kwargs.enable_thinking=false` (llama.cpp / vLLM), as `thinking: disabled` for Anthropic, `reasoning.effort: none` for Responses, `think: false` for Ollama.
